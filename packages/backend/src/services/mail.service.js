...
async function getLogoAttachment() {
  ...
  try {
    ...
    if (!logoPath) {
      console.warn('[mail] logo file not found — email will use text header.');
      return null;
    }
...
function buildContactEmailText(submission) {
  ...
  return [
    'BrandSpire — New Contact Form Lead',
    '',
...
export async function sendContactNotification(submission) {
  if (!isMailConfigured()) {
    console.warn('[mail] SMTP not configured — skipping Gmail notification.');
